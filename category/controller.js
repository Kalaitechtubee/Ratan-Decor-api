...
const getCategoryTree = async (req, res) => {
  try {
    ...
    const tree = buildTree(mappedCategories, null, req); // FIXED: Pass req for URLs
  ...
  try {
    ...
    res.status(201).json({
      ...
      category: {
        ...
        brandName: createdCategory.brandName,
        image: null,
        imageUrl: null,
...
const updateCategory = async (req, res) => {
  ...
  try {
    ...
    // Handle brandName update
    if (brandName !== undefined) {
      const newBrandName = brandName.trim() || null;
      if (newBrandName !== category.brandName) {
        updateData.brandName = newBrandName;
        hasChanges = true;
      }
    ...
    if (!hasChanges) {
      ...
      return res.status(200).json({
        ...
        category: {
          ...
          brandName: noChangeCategory.brandName,
...
const createCategory = async (req, res) => {
  ...
  try {
    ...
    const category = await Category.create({
      name: trimmedName,
      parentId: null,
      brandName: brandName?.trim() || null, // FIXED: Trim if provided
      image: imageFilename,
    }, { transaction });
