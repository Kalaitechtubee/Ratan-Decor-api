...
const getProducts = async (req, res) => {
    try {
        ...
        const userRole = ProductService.getReqUserRole(req);
        const whereClause = {};
        ...
        if (userType) {
            whereClause[Op.and] = sequelize.where(
                sequelize.fn('LOWER', sequelize.col('visibleTo')),
                { [Op.like]: `%"${userType.toLowerCase()}"%` }
            );
        }
        ...
        const processedProducts = products.map(product => {
            const productData = ProductService.processProductData(product, req);
            productData.price = ProductService.computePrice(product, userRole);
            return productData;
        });
...
const createProduct = async (req, res) => {
    try {
        ...
        const parsedSpecifications = ProductService.safeJsonParse(specifications, {});
        const parsedVisibleTo = ProductService.safeJsonParse(visibleTo, ['Residential', 'Commercial', 'Modular Kitchen', 'Others']);
        const parsedColors = ProductService.safeJsonParse(colors, []);

        if (parsedVisibleTo && !ProductService.validateVisibleTo(parsedVisibleTo)) {
            return res.status(400).json({
                message: 'Invalid visibleTo values: must be a non-empty array',
                received: parsedVisibleTo
            });
        }
        if (parsedColors && !ProductService.validateColors(parsedColors)) {
            return res.status(400).json({
        ...
        res.status(201).json({
            ...
            product: ProductService.processProductData(createdProduct, req)
        });
    } catch (error) {
...
const getProductByName = async (req, res) => {
    try {
        ...
        processedProduct.price = ProductService.computePrice(product, userRole);
        res.json({
...
const updateProduct = async (req, res) => {
    try {
        ...
        if (req.body.keptImages) {
            keptImages = ProductService.safeJsonParse(req.body.keptImages, []);
        }
        ...
        const parsedSpecifications = ProductService.safeJsonParse(specifications);
        const parsedVisibleTo = ProductService.safeJsonParse(visibleTo);
        const parsedColors = ProductService.safeJsonParse(colors);
        if (parsedVisibleTo !== undefined && !ProductService.validateVisibleTo(parsedVisibleTo)) {
            return res.status(400).json({ message: 'Invalid visibleTo values: must be a non-empty array' });
        }
        if (parsedColors !== undefined && !ProductService.validateColors(parsedColors)) {
        ...
        res.json({
            ...
            product: ProductService.processProductData(updatedProduct, req)
        });
    } catch (error) {
...
const updateProductAll = async (req, res) => {
    try {
        ...
        let subcategoryId = req.body.subcategoryId;
        let keptImages = ProductService.safeJsonParse(req.body.keptImages, []);
        ...
        const parsedVisibleTo = ProductService.safeJsonParse(visibleTo, []);
        const parsedColors = ProductService.safeJsonParse(colors, []);
        if (!ProductService.validateVisibleTo(parsedVisibleTo)) {
            return res.status(400).json({ message: 'Invalid visibleTo values: must be a non-empty array' });
        }
        if (!ProductService.validateColors(parsedColors)) {
            return res.status(400).json({ message: 'Invalid colors: must be an array of non-empty strings' });
        }
        if (gst !== undefined && (isNaN(gst) || gst < 0 || gst > 100)) {
            return res.status(400).json({ message: 'Invalid GST: must be a number between 0 and 100' });
...
const deleteProduct = async (req, res) => {
    try {
        ...
        if (filesToDelete.length > 0) {
            await ProductService.cleanupFiles(filesToDelete, uploadDir);
        }
...
const addProductRating = async (req, res) => {
    try {
        ...
        if (!ProductService.validateRating(rating)) {
